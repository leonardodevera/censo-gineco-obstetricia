...
function section(scope,list,label,breakBefore){
 const now=new Date();
 return '<section class="handoffPart'+(breakBefore?' handoffPartBreak':'')+'"><div class="hospitalPrintHead"><b>ENTREGA DE GUARDIA · GINECO-OBSTETRICIA</b><span>'+esc(scope.name)+' · '+esc(label)+' · '+now.toLocaleDateString('es-EC')+' '+now.toLocaleTimeString('es-EC',{hour:'2-digit',minute:'2-digit'})+' · '+list.length+' pacientes</span></div>'+tableFor(list)+'</section>';
}
