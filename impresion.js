...
function flags(b){const a=[];if(b.postIcu||b.post_icu)a.push('POST-UCI');if(b.isolation)a.push('AISLAMIENTO');if(b.closeWatch||b.close_watch)a.push('VIGILANCIA');return a.length?'<div class="printFlags">'+a.map(esc).join(' · ')+'</div>':''}
...
@media print{@page{size:A4 landscape;margin:0}.pdfSheet.hospitalSheet{width:297mm!important;min-height:210mm!important;padding:6mm!important}.hospitalPrint{font-size:6.8pt!important}.hospitalPrintHead{font-size:7.5pt!important}}
`;document.head.appendChild(s)}
function install(){if(typeof showPdfPreview!=='function'||!document.getElementById('pdfSheet'))return false;addPrintStyle();showPdfPreview=function(){const sheet=$('pdfSheet');sheet.className='pdfSheet hospitalSheet';sheet.innerHTML=hospitalPrint();$('pdfModal').style.display='block'};return true}
